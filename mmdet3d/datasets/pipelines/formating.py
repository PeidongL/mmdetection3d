...
class Collect3D(object):
    ...
    def __init__(
            ...
            meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img', 
                       'lidar2camera', 'camera_intrinsics', 'img_feature', 'side_img_feature', 'side_img_feature', 'img_feature_shape',
                       'depth2img', 'cam2img', 'pad_shape', 'scale_factor', 'flip',
                       'pcd_horizontal_flip', 'pcd_vertical_flip', 'box_mode_3d',
...
class DefaultFormatBundleMultiCam3D(object):
    ...
    def _format_multi_cam_imgs(self, results):
        ...
        if 'rear_img_feature' in results and len(results['rear_img_feature']) > 0:
            feature_list = []
            for idx, feat in enumerate(results['rear_img_feature']):
                feature_list.append(feat)
            
            feats = np.ascontiguousarray(np.stack(feature_list, axis=0))
            results['rear_img_feature'] = DC(to_tensor(feats), stack=True)        
