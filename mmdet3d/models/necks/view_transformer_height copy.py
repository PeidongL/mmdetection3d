...
class IHRLayer(nn.Module):
    ...
    def forward(self, inputs, bev_queries, bev_pos, reference_points, point_cloud_range, 
                rot, tran, intrin, post_rot, post_tran, bda, image_shapes, bev_h, bev_w, depth_prob=None, prob_range=None):
        #B, HW, 3
        ...
        output, bev_mask = feature_sampling(
            inputs, reference_points, depth_prob, prob_range, None, point_cloud_range, rot, tran, intrin, post_rot, post_tran, bda, image_shapes)
...
def feature_sampling(feat, reference_points, depth_prob, prob_range, img_offset, pc_range, rot, tran, intrin, post_rot, post_tran, bda, image_shapes):
...
class LSSViewTransformerBEVHeightDepth(HeightTrans):
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        for ihr in self.ihr_layers:
            output, ref_points = ihr(tran_feat, bev_queries, bev_pos, ref_3d, self.pc_range, rot, tran, intrin, 
            post_rot, post_tran, bda, self.data_config['input_size'], self.bev_h, self.bev_w, depth, self.grid_config['depth'])
            bev_queries = output 
            ref_3d = ref_points
        output = output.permute(0, 2, 1).contiguous().view(B, -1, self.bev_h, self.bev_w)
